...
import { PLAY_CARDS } from "@/lib/navConfig";
    ...
    <AppLayout>
      <div className="max-w-3xl mx-auto px-4 py-6">
        <header className="mb-6">
          <h1 className="font-serif text-2xl tracking-tight">{t("play.title")}</h1>
          <p className="text-sm text-muted-foreground mt-1">{t("play.subtitle")}</p>
        </header>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {PLAY_CARDS.map((card) => (
            <Link key={card.href} href={card.href}>
              <div
                data-testid={`card-play-${card.href.replace(/\W+/g, "-")}`}
